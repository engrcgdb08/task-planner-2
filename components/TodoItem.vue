        ...
        <li class="choice-list__item task-card-item">
          ...
          <span class="choice-list__aside">
            ...
            <div v-show="pop === true" class="box">
              ...
              <ul class="" style="margin: 0.5em 0 0 -3em">
                <li
                  class="task-card-item"
                  v-for="(user, i) in searchResultsList"
                  :key="i"
    ...
    searchDataFilter() {
      ...
      if (this.searchTasks.toLowerCase() !== "") {
        const results = data.filter((item, i) => {
          if (
            item.name.toLowerCase().indexOf(this.searchTasks.toLowerCase()) > -1
          ) {
            return item;
          }
        });
